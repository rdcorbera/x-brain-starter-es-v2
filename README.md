...
**En curso** — corte 1, paso 8: los skills reescritos para invocar `./brain`. Hechos `/x-setup`, que pasa de una entrevista de 30 minutos a elegir un **profile de rol** y ajustarlo en unos cinco; `/x-nueva-iniciativa`, que abre un proyecto con su plan y su documentación inicial ya procesada — y deja que `GOALS.md` se genere solo; y `/x-consultar`, que responde con la fuente de cada afirmación, ofrece archivar la síntesis y **registra la consulta** en un log nuevo. Quedan 13 skills, y después la migración del cerebro en producción.
